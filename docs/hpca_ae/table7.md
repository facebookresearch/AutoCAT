...
You can see that at epoch **6** which is the **7th** epoch, the training ```correct_rate``` jump to 0.98736399. And at th end of the training, the evaluation ```episode_length``` is 6.98. For baseline and PLcache, we perform three experiments each and calculate the corresponding average epoch to converge and final evaluation episode_length at the end.
